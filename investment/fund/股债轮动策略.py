...
def get_fund(code, start_date=str(date.today()-timedelta(60)), end_date=str(date.today()), page=1, per=20):
    # 获取html
...
if __name__ == '__main__':
    funds = [
        ...
        {"name": "食品饮料ETF", "code": 515170},
    ]
    # 查询两只产品的相关性
    # data1 = get_fund(513050)
    # data2 = get_fund(159967)
    # print(round(correlation(data1, data2), 2))
    for fund in funds:
        applies = get_fund(fund.get("code"))[-22:]
        fund_month_applies = round(sum(applies), 2)
        print(f"{fund.get('name')}: {fund_month_applies}")
