...
class Stock_BaseInfo:
    def __init__(self, code):
        ...
        self.momentum = Momentum(code)


    def __str__(self):
        return f"代码:{self.code}\t名称:{self.name}\t行业:{self.industry}\tPE:{self.pe}\tPB:{self.pb}" \
               f"\t收入同比:{self.rev_yoy}\t利润同比:{self.profit_yoy}\t毛利率:{self.gpr}\t净利润率:{self.npr}\t动量值:{self.momentum.momentum_value}\t12个月累计收益:{self.momentum.cumulative_yield}\t超额收益:{self.momentum.excess_yield}"
...
class Momentum:
    ...
    def cumulative_run(self):
        data_week = self.get_stock_kline(period=102, limit=54)
        if data_week and len(data_week)>1:
        ...
        if data_month and len(data_month) > 13:
