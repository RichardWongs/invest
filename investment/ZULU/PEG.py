...
def get_quarter_report():
    ...
    print(quarter_report_list)
    target_file = f"quarter_report.bin"
...
def get_earnings_forecast():
    ...
    for pageNumber in range(1, 5):
        timestamp = int(time.time()*1000)
        callback = f"jQuery1123011263307199378958_{timestamp}"
        url = f"http://datacenter-web.eastmoney.com/securities/api/data/v1/get?callback={callback}&sortColumns=NOTICE_DATE%2CSECURITY_CODE&sortTypes=-1%2C-1&pageSize={pageSize}&pageNumber={pageNumber}&reportName=RPT_PUBLIC_OP_NEWPREDICT&columns=ALL&token=894050c76af8597a853f5b408b759f5d&filter=(REPORT_DATE%3D%27{_date}%27)"
        response = requests.get(url)
        response = response.text.replace(f"{callback}(", '')[:-2]
        response = json.loads(response)
        if 'result' in response.keys():
            ...
            if response:
                if 'data' in response.keys():
                    ...
                    for i in response:
                        if i['SECURITY_CODE'][0] in ('0', '3', '6') \
                                and i['SECURITY_CODE'] not in earnings_forecast.keys():
                            earnings_forecast[i['SECURITY_CODE']] = i
    target_file = f"earnings_forecast.bin"
    if target_file in os.listdir(os.curdir):
        os.remove(target_file)
    with open(target_file, 'wb') as f:
        f.write(pickle.dumps(earnings_forecast))
