...
def run_V2():
    ...
    data = pd.read_csv(f'daily_data_V3.csv', encoding='utf-8', index_col='trade_date')
    data.index = pd.to_datetime(data.index, format='%Y%m%d', errors='ignore')
    for rps_day in rps_days:
        ret = cal_ret(data, w=rps_day)
        rps = all_RPS(ret)
        ...
        for k, v in rps.items():
            ...
            new_rps[k] = tmp
        fill_in_data_V2(new_rps, filename=f'RPS_{rps_day}_V3.csv')
    end = int(time.time())
    minutes = int((end - start) / 60)
    seconds = (end - start) % 60
